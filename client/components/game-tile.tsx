...
interface LetterTileProps {
  letter: string;
  team?: "a" | "b";
  state?: "idle" | "placed" | "selected" | "selected-hover";
  canPlace?: boolean;
  zoom?: number;
  onClick?: () => void;
}
...
export default function GameTile({ letter, team, state = "idle", zoom = 1, onClick }: LetterTileProps) {
  ...
  const isHoverGhost = state === "selected-hover";
  ...
  return (
    <motion.div
      ...
      className={`flex items-center justify-center font-extrabold select-none transition-all duration-150 border-2
        ...
        ${
          isHoverGhost
            ? "rounded-lg font-mono border-dashed border-primary/50 bg-primary/10 text-primary/60 pointer-events-none"
            : state === "selected"
